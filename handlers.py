...
def card_detected(self, card):
	if self.verbosity == 1:
		print
		'Detected: %s (%.2f secs)' % (card.name, card.detected_in)
	elif self.verbosity >= 2:
		print
		'detection successful, %e, %d, %s' % (card.detected_in, len(card.poll_results), card.name)
	self.count += 1
...
def detector_gave_up(self, args):
	if self.verbosity == 1:
		print
		'Detection taking too long, giving up'
	elif self.verbosity >= 2:
		print
		'detection timed out,,,'
...
def detector_stopped(self):
	print
	'Detector stopped. Detected %s cards' % self.count
...
class OutputFileHandler(EventHandler):
	...
	def _init_deckbox_org_csv(self):
		...
		self.writer.writerow(
			['Count', 'Tradelist Count', 'Name', 'Foil', 'Textless', 'Promo', 'Signed', 'Edition', 'Condition',
			 'Language'])
